...
#global static_back = None

#keras.backend.tensorflow_backend.set_session(sess)
# def onmouse(event,x,y,flags,param):
# 	if event== cv2.EVENT_LBUTTONDOWN:
# 		print(x,y)
def motion_check(static_back,frame):
	# check for diffence for start and stop program
	static_back
	gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY) 
	gray1=cv2.cvtColor(static_back, cv2.COLOR_BGR2GRAY)
	...
	img2=gray1-gray
	motion=np.argmax(img2)

	print("img_dif",motion)
...
def img2url(img):
	# OpenCV to PIL
	#image to url conver for crop img sent to database
...
# 		# Drawing a rectangle on c
	...
	print("cropedx",q)
	print("cropedy",t)
	cv2.setMouseCallback('cropped',onmouse)
...
#img=cv2.VideoCapture("dsp-dir-3.mp4")
frame_prev=None
...
while True:
	for i in range(skip + 1):
		...
		image3=img.read()[1]
		try:
			image3=cv2.resize(image3,(1190,700))
		except:
			pass
	cv2.imshow("",image3)
	cv2.waitKey(1)
	...
	diff=0
	motion=motion_check(static_back,image3)
	if motion> 1500:
		start1=time.time()
		#end=time.
		diff=int(motion)
	...
	## (2) Detect circles
		...
		# 		# Drawing a rectangle on c
			...
			diff_time=abs(start1-end)
			if diff_time<1:
				val_x.append(x)
				val_y.append(y)
				i+=1
			if len(val_x)>0:
				cx1=int(sum(val_x)/len(val_x))
				cy1=int(sum(val_y)/len(val_y))
				cx2=cx-.1*cx
				cx3=cx+ .1*cx
				cy2=cy-.1*cy
				cy3=cy+ .1*cy
			...
			cy=params['cy']
			print(cx,cy)
			croped = image3[cy-172:cy+172,cx-174:cx+174].copy()
			...
			# cols=h
			#cv2.imshow("",croped)
			dst = cv2.warpPolar(croped, (int(h*3.5),int(w*3)),(180,180),radius2, flags=(cv2.WARP_POLAR_LOG))
			dst1 = cv2.warpPolar(croped1, (int(h*3.5),int(w*3)),(180,180),radius2, flags=(cv2.WARP_POLAR_LOG))
			...
			#cv2.imshow("",edges)
			cv2.imwrite("new1.jpg",crop1)
			cv2.imwrite("new2.jpg",crop)


			text =  pytesseract.image_to_string(crop1,config='--psm 11--oem 3 -c tessedit_char_whitelist=0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ') 
			print("d",text)
			...
			img_path="{}/full_image{}.jpg".format(full_img_path,p)

			rotated_img_path="{}/rotated{}.jpg".format(full_img_path,p)
			rotated_img_path1="{}/rotated1{}.jpg".format(full_img_path,p)

			print(img_path)

			cv2.imwrite(img_path,image)
			cv2.imwrite(rotated_img_path,rotated)
			cv2.imwrite(rotated_img_path1,rotated1)
			p+=1
			k=+1
			#print(ID)
