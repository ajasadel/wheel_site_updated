...
while True:
	...
	diff=0
	motion=motion_check(static_back,image)
	if motion> 500:
		start1=time.time()
		diff=motion
# if motion<=5:
	...
	## (2) Detect circles
		...
		# 		# Drawing a rectangle on c
			...
			print("*T")
			print(cx,cy)
			croped = image[cy-150:cy+150,cx-150:cx+150].copy()
	...
	## (2) Detect circles
		...
		# 		# Drawing a rectangle on c
			...
			cy4=croped_cy
			if len(val_x1)>0:
				cx1=int(sum(val_x1)/len(val_x1))
				cy1=int(sum(val_y1)/len(val_y1))
				cx2=croped_cx-.1*croped_cx
				cx3=croped_cx+ .1*croped_cx
				cy2=croped_cy-.1*croped_cy
				cy3=croped_cy+ .1*croped_cy
				if cx2<cx1<cx3:
					cx4=cx1
				if cy2<cy<cy3:
					cy4=cy1
			...
			#cv2.imshow("",croped)
			#print(cx4,cy4)
			dst = cv2.warpPolar(croped, (int(h*3.5),int(w*3)),(cx4,cy4),radius2, flags=(cv2.WARP_POLAR_LOG))
			dst1 = cv2.warpPolar(croped1, (int(h*3.5),int(w*3)),(cx4,cy4),radius2, flags=(cv2.WARP_POLAR_LOG))
